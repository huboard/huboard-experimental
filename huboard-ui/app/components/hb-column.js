import Ember from "ember";
import SortableMixin from "../mixins/sortable";
import CardOrderMixin from "../mixins/card-order";
import sortablejs from 'npm:sortablejs';

Function.prototype.__log = function(message, context){
  var fn = this;
  return function(){
    console.log('before', message, context, arguments);
    var result = fn.apply(this,arguments);
    console.log('after', message, context, arguments);
    return result;
  }
}

var ColumnComponent = Ember.Component.extend(CardOrderMixin, {
  board: Ember.inject.service(),
  classNames: ["column"],
  cards: Ember.A(),

  issues: function(){
    var column = this.get("column");
    var issues = this.get("board.combinedIssues");
    return issues.filter(function(i){
      return i.column.index === column.index;
    }).sort(this.sortStrategy);
  }.property("board.model.combinedIssues.@each.{columnIndex,order}"),

  moveIssue: function(issue, order){
    var self = this;
    this.get("issues").removeObject(issue);
    Ember.run.schedule("afterRender", self, function(){
      issue.set("column", self.get("column"));
      issue.set("_data.order", order);
    });
  },
  sortStrategy: function(a,b){
    return a._data.order - b._data.order;
  },
  register: function(){
    this.sendAction("registerColumn", this);
  }.on("didInsertElement"), 
  attachSortable: function(){
    var column = this,
      list = this.$('.cards');

    this.$('.cards').each(function(){
      var sortable = new sortablejs(this,{
        group: 'column',
        ghostClass: 'ui-sortable-placeholder',
        setData: function(){}.__log('setData'),
        onStart: function(){}.__log('onStart'),
        onEnd: function(){}.__log('onEnd'),
        onAdd: function(){}.__log('onAdd'),
        onUpdate: function(){}.__log('onUpdate'),
        onSort: function(evn){
          var elements = list.find("> li"),
            element = evn.item,
            index = elements.index(evn.item);

          if(index === -1) { return; }

          var registry = column.container.lookup('-view-registry:main');
          var order = column.calculateOrder(evn, element, elements, registry);

          // here be dragons
          var component = registry[$(evn.item).attr('id')]

          column.moveIssue(component.get('issue'), order)
        }.__log('onSort', this),
        onRemove: function(){}.__log('onRemove'),
        onFilter: function(){}.__log('onFilter'),
        onMove: function(){}.__log('onMove'),
      })
    });

  }.on('didInsertElement')
});

export default ColumnComponent;
